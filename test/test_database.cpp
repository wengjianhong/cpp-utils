...
TEST(DatabaseConnectionTest, SqliteConnectAndQuery) {
  cpputils::database::ConnectionConfig opts;
  opts.database_type = cpputils::database::DatabaseType::kSqlite3;
  opts.conn_string = "dbname=:memory:";

  auto connection = cpputils::database::CreateConnection(opts);
...
TEST(DatabaseConnectionTest, TransactionRollback) {
  ...
  ASSERT_TRUE(connection->RollbackTransaction());
...
TEST(DatabaseConnectionTest, NotConnectedReturnsError) {
  auto connection = cpputils::database::CreateConnection(cpputils::database::ConnectionConfig{});
  ASSERT_NE(connection, nullptr);
...
TEST(DatabaseConnectionPoolTest, AcquireAndQuery) {
  cpputils::database::SqliteConfig sqlite_cfg;
  sqlite_cfg.database_path = ":memory:";

  auto pool = cpputils::database::CreateConnectionPool();
  ASSERT_TRUE(
    pool->Open(cpputils::database::ConnectionPoolConfig{cpputils::database::ConnectionConfig{sqlite_cfg}, 2}));
